...
public struct MetadataChange
{
    ...
    public static MetadataChange operator -(MetadataChange rhs)
    {
        return new MetadataChange(-rhs.components, -rhs.voxels, -rhs.nonNullVoxels, -rhs.volume, -rhs.nonNullVolume);
    }
}
...
public abstract class OctreeComponent<T>
{
    ...
    /// <summary>
    /// Gets stored point at Voxel containing point.
    /// <summary>
    ...
    {
        //Debug.Log(string.Format("Contains: point: {0}, min: {1}, max:{2}, minSize: {3}",
          //  point.ToString(), min.ToString(), max.ToString(), minSize));
...
public class OctreeContainer<T> : OctreeComponent<T>
{
        ...
        : base(myMin, myMax, myMinSize)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            children[i] = new Voxel<T>(voxBounds["min"], voxBounds["max"], minSize);
        }
    }
    ...
    /// Gets stored point at Voxel containing point.
    /// <summary>
    public override Vector3 getPoint(Vector3 pointToGet)
    {
        if (!contains(pointToGet))
            throw new ArgumentOutOfRangeException("pointToGet", "not contained in Container.");
        int childNum = whichChild(pointToGet);
